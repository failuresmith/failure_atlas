...
Build a minimal abstract failure harness where each failure mode is:
1) reproducible,
2) detectable,
...
### 7) Abstract over concrete
- Use generic mechanism language first: state, transitions, faults, invariants, recovery
- Avoid product-like metaphors unless essential for mechanism legibility
...
- `runtime/`
  - minimal reusable primitives (state, transitions, effects boundary, deterministic clock)
...
- Keep FM_001 abstract: avoid domain-specific workflow detail unless required to expose the mechanism
