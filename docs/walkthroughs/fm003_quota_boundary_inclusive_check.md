# FM_003 Walkthrough — Invariant → Repro → Guardrail
...
2) **Reproduce failure** — `lab/failure_modes/FM_003_quota_boundary_off_by_one/tests/test_repro_fm003.py` shows `max + 1` allocations admitted.
...
5) **Prove fix** — `tests/test_prevent_fm003.py` confirms boundary holds for multiple max values.
6) **Happy path** — `tests/test_fm003_happy_path.py` confirms inclusive guardrail does not block valid capacity.

## How to run
```bash
cd lab
pytest failure_modes/FM_003_quota_boundary_off_by_one/tests -q
```
