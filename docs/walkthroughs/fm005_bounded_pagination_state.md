# FM_005 Walkthrough — Invariant → Repro → Guardrail
...
2) **Reproduce failure** — `lab/failure_modes/FM_005_unbounded_pagination_state/tests/test_repro_fm005.py` shows cookie count exceeds budget when enforcement is off.
...
5) **Prove fix** — `tests/test_prevent_fm005.py` verifies cookie count stays within budget and tracks evictions.
6) **Happy path** — `tests/test_fm005_happy_path.py` ensures bounded mode doesn’t break normal pagination.

## How to run
```bash
cd lab
pytest failure_modes/FM_005_unbounded_pagination_state/tests -q
```
